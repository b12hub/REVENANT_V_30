...
    
   
  | **Internal service-to-service calls** |   **gRPC + Protocol Buffers** | Typed, fast, and the contract is enforced by the compiler on both ends — the spiritual successor to the "Immutable Envelope" contract discipline from the n8n-era roadmap, except now it's actually unbreakable rather than just well-documented. |  
   
    
   
     
...
    
   
       Shared --> Compliance[Compliance/AML Service]  
   
    
   
...
    
   
### **4.1 Tenant Isolation & Security Framework**  
The core shift from a bespoke single-bank deployment to a multi-tenant platform requires strict horizontal isolation across the stack.  
[v31 Monolith Platform] -> Hardcoded Database Path / Single Vault Namespace  
                                   │  
                                   ▼  
[v32 Multi-Tenant Platform] -> Router/Gateway identifies X-Tenant-ID  
                                   ├── Vault Path: /secret/data/tenant_x/*  
                                   └── Database: Row-Level Security (RLS) or Isolated Schema  
   
- **Configuration Namespace Isolation (F10):**  
  - *v31 Mechanics:* Environmental variables and Vault paths are globally defined or hardcoded to one bank's credentials.  
  - *v32 Migration:* Every incoming API request and worker execution must carry a signed X-Tenant-ID header. Vault configuration paths must resolve dynamically to /secret/data/tenant_id/*.  
- **Database Multi-Tenancy (F10):**  
  - *v31 Mechanics:* Single database schema with unstructured tables.  
  - *v32 Migration:* Implementation of Postgres Row-Level Security (RLS) or separate database schemas per tenant, controlled by the connection pooling layer (e.g., PgBouncer tenant routing).  
- **Anonymized Fraud Intelligence Network (F11):**  
  - *v31 Mechanics:* Fraud signals (malicious voice prints, device IDs) are completely isolated within a single client's instance.  
  - *v32 Migration:* A global, decentralized database cluster storing cryptographically hashed, non-invertible identification tokens ($SHA-256$). When a fraud vector is flagged, its hash is written to the global pool, triggering instantaneous real-time caching across all tenants' local Redis instances.  
   
...
     
   
  **Phase 1 — Shadow Mode (Build Trust Before Touching Real Traffic)**  
   
    
   
  Put the new API Gateway in front of n8n, but for now it changes nothing for the customer: **every request still gets forwarded to the existing n8n webhook, unchanged**, and that response is what the customer actually receives. At the same time, the Gateway *also* sends an asynchronous copy of each request to the new code-first services — their output is logged and compared against n8n's, but never shown to anyone. Think of it like a co-pilot quietly checking the real pilot's work without ever touching the controls. This phase proves the new FAQ and Bill-Pay services produce the    *same* decisions n8n does, with zero customer-facing risk, before a single real request depends on them.  
   
  **Phase 2 — Cut Over the Low-Risk Domains First**  
   
    
   
...
   
  **Phase 3 — Cut Over the High-Risk Domains, Carefully**  
   
    
   
