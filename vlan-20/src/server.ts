...
import type { FAQResponseModel, FAQStatus } from './types.js';
...
//
// Deliberately NOT pino, despite pino being used in VLAN 0's gateway —
// this service's explicit dependency list was scoped to uuid + dotenv plus
// the gRPC/Zod stack, and a low-latency stateless pipe with a single log
// call per request doesn't need pino's buffering/transport machinery to
// satisfy "clean, production-grade logging." console.log with a fixed JSON
// shape gets the same observability outcome (structured, greppable,
// trace-correlated) with zero additional dependency surface.
// ---------------------------------------------------------------------------
...
interface GetFAQAnswerCall {
  request: Record<string, unknown>;
}

type GetFAQAnswerCallback = (
 ...
 * header.
 */
function buildFallbackAnswer(language: string): string {
...
function buildGetFAQAnswerHandler() {
  return function getFAQAnswer(call: GetFAQAnswerCall, callback: GetFAQAnswerCallback): void {
...
function buildServer(): grpc.Server {
  ...
  const loaded = grpc.loadPackageDefinition(packageDefinition) as unknown as {
    revenant: { faq: { v1: { FAQService: grpc.ServiceClientConstructor } } };
  };
