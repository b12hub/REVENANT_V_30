...
interface AICognitionServiceClient extends grpc.Client {
  GenerateAdvisory(
    request: {
      trace_id: string;
      tenant_id: string;
    ...
    callback: (
      ...
      response?: { answer: string; route: string; confidence: number },
    ) => void,
  ): grpc.ClientUnaryCall;
}

function loadAICognitionClientConstructor(): new (
  address: string,
  credentials: grpc.ChannelCredentials,
) => AICognitionServiceClient {
...
export class AICognitionClient {
  private readonly client: AICognitionServiceClient;
  ...
  async generateAdvisory(
    request: GenerateAdvisoryRequestModel,
  ): Promise<GenerateAdvisoryResult> {
    return new Promise((resolve) => {
      ...
      this.client.GenerateAdvisory(
        ...
        { deadline: Date.now() + ABSOLUTE_DEADLINE_MS },
        (error, response) => {
          if (error) {
